...
Let us illustrate how to visualize a set of .bam samples. We donwloaded .bam files of 1038 male samples diagnosed with prostate cancer from [`TCGA project`](https://cancergenome.nih.gov/). Processed .bam files (using `madseqloy` function) can be loaded by:
...
The method is indicating that there are no samples having a LOY.
...
plotIndSNP(file.path(PathHapMapLRR, "TCGA-CF-A3MI-01A.txt"))
```
