...
text(cex=1, x=c(1:5)-.25, y=-1.25, PRADresBdev$class$orig[1:5], xpd=TRUE, srt=45, pos=2)
...
legend("topright", legend = c("p arm adjusted-p", "p arm only nominal-p", "q arm adjusted-p", "q arm only nominal-p"), pch = c(20, 1, 20, 1), col=c("red", "red", "black", "black"))
```
...
for (i in samples){
  ...
  plotIndSNP(PRADres, i, LRRCol = 5, BAFCol = 4)  
  dev.off()
}
...
for (i in samples){
  MADloy:::plotIndSNP(res, i, LRRCol = 5, BAFCol = 4)  
}
