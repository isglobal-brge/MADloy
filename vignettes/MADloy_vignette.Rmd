...
if(!file.exists("./temp"))
dir.create("./temp")
filedata <- "https://drive.google.com/uc?export=download&id=0B_pkfs1Zxa9DRkhUZTY2OV80ZDg" ## Where 0B8VpqcZ29oqOY09uNEVKMk5fQW8 is the identifier of the shared file in google drive
if(Sys.info()["sysname"] == "Linux") download.file(filedata, destfile = "temp/MADSeqLOY_BLCA.rda", method="wget")
if(Sys.info()["sysname"] == "Windows") download.file(filedata, destfile = "temp/MADSeqLOY_BLCA.rda", mode = "wb")
...
```{r, plot_BLCA, fig.show='hold', fig.width=6, fig.height=5, fig.cap = "Mean coverage of samples belonging to BLCA data."}
plot(BLCA)
```
...
LRR can be downloaded from TCGA since this information is considered of level 3 and it is available from TCGA Data Portal. The plot of mY region of that sample can be obtained by simply typing (NOTE: The function `plotIndSNP` can procude this plot from: 1) an object of class `madloy`; 2) a matrix having chromosome, position and LRR; or 3) a path indicanting where a file having chromosome, position and LRR is located).


```{r, LRR_BLCA_plot, fig.show='hold', fig.width=6, fig.height=5, fig.cap = "LRR in the mY region of BLCA sample `r id.tcga.low`. "}
folder <- "../../BLCAdata/rawData"
plotIndSNP(file.path(folder, paste0(id.tcga.low, ".txt")))
```
...
```{r, fig.show='hold', fig.width=6, fig.height=5,fig.cap = "Plot of a LOY object obtained from BLCA dataset", eval=FALSE}
plot(ans2)
```
