...
The method to detect the altered values is different for each type of data. **Marcos, esto no se a que te refieres** **Me refiero a que el método para detectar los valores alterados es diferente para los datos de secuencia y para los datos de array**
...
pos.id <- grep(id.tcga.low, names(blca.lrr))
plotIndSNP(blca.lrr, rsCol = 1, ChrCol = 2, PosCol = 3, LRRCol = pos.id)
```
