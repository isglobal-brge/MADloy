...
```{r, fig.show='hold', fig.width=6, fig.height=4.5, fig.cap = "Plot of MADloy object including 89 HapMap samples"}
...
```{r, fig.show='hold', fig.width=6, fig.height=4.5, fig.cap = "Plot of a MADloy object with 23 HapMap 3 male samples analyzed"}
...
First, let's have a look at the summarized data of LRR in mY region and in the autosomal choromosomes that are used as the reference ones
...
```{r, plot_lrr_ref_mY, fig.show='hold', fig.width=6, fig.height=9, fig.cap ="LRR in reference (autosomes) and mY region of BLCA data"}
par(mfrow=c(2,1))
plot(lrr.summary[,2], xlab="samples", ylab="LRR in autosomes")
abline(h=0, lty=2, col="red")
plot(lrr.summary[,1], xlab="samples", ylab="LRR in mY region")
abline(h=0, lty=2, col="red")
```

Here we can observe that:

> * The LRR in autosomes are centered in 0 as expected
> * The LRR in mY region deviates from 0 since there is only one copy (e.g ploidy is equal to 1) and hence, the LRR is centered in 2/3·log(1/2) = `r round(2/3*log(1/2),2)`
> * There is also a little deviation due to the overall mean in the array

Therefore, data in mY region is normalized by computing 

```{r, normalized}
norm.lrr <- lrr.summary[,1] - lrr.summary[,2] - 2/3*log(1/2)
``` 

Now the normalized LRR in mY regions is properly centered at 0

```{r, plot_normalized, fig.show='hold', fig.width=6, fig.height=5, fig.cap ="Normalized LRR in mY region of BLCA data"}
plot(norm.lrr, ylab="Normalized LRR in mY region", xlab="samples")
abline(h=0, lty=2, col="red")
``` 
