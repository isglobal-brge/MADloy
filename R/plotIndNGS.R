...
#' @export
# TODO Set optimal png parameters to plot.  TODO Optimize and improve
# performance.
plotIndNGS <- function(x, sample, label, file, readmax = 1000, logb = 10, rad = 1.5, 
    ...
    lreads <- log(pmax(1, rawreads), logb)  ## Minimum value set to 1 to calculate log10
    ml <- min(lreads)
    lreads <- lreads - ml
    ...
    lTargMedianCov <- lTargMedianCov/(log(readmax, logb) - ml)  #Log Scaled
    ...
    lRef1MedianCov <- lRef1MedianCov/(log(readmax, logb) - ml)  #Log Scaled
    ...
    lRef2MedianCov <- lRef2MedianCov/(log(readmax, logb) - ml)  #Log Scaled
    
    ## Check if png
    if (!missing(file)) 
        grDevices::png(filename = file, width = 150, height = 150, units = "mm", 
    ...
    ## Draw a circle
    graphics::plot(NULL, xlim = c(-2.7, 2.7), ylim = c(-2.7, 2.7), axes = FALSE, 
        ann = FALSE)
    ...
    plotrix::draw.circle(0, 0, rado, nv = 100, border = "white", col = "white", lty = 1, 
        lwd = 1)
    ...
    probeRanges <- IRanges::ranges(x$par$targets[IRanges::overlapsAny(x$par$targets, 
        x$par$target.region)])
    
    for (i in 1:length(probeRanges)) {
        end = end(probeRanges)[i]
        start = start(probeRanges)[i]
        ecolitk::polygonArc(theta0 = pi/2 + 2 * pi * start/kbmax, theta1 = pi/2 + 
            2 * pi * end/kbmax, radius.in = 1.25, radius.out = rado, edges = 1000, 
            col = "black", border = "black", lwd = 1)
    }
    
    ## Draw Medians
    ecolitk::linesCircle(rado + lTargMedianCov, lty = 2, lwd = 3, col = "green")
    tlist <- c(tlist, paste0("Target median coverage= ", rawTargMedianCov))
    clist <- c(clist, "green")
    ltylist <- c(ltylist, 2)
    
    ecolitk::linesCircle(rado + lRef1MedianCov, lty = 2, lwd = 3, col = "red")
    tlist <- c(tlist, paste0("Reference 1 median coverage = ", rawRef1MedianCov))
    clist <- c(clist, "red")
    ltylist <- c(ltylist, 2)
    
    ecolitk::linesCircle(rado + lRef2MedianCov, lty = 2, lwd = 3, col = "yellow")
    tlist <- c(tlist, paste0("Reference 2 median coverage = ", rawRef2MedianCov))
    ...
    for (kb in kbs) {
        ...
        dist <- trad + strw
        graphics::text(dist * cos(theta), dist * sin(theta), paste(kb/1e+06, " mb", 
            sep = ""), col = "black", cex = 1 * cexnum, srt = 360 * ttext/(2 * pi), 
            adj = c(0, 0))
    }
    ...
    ## Add Legend
    graphics::legend("bottomright", legend = tlist, lty = ltylist, lwd = 2, col = clist, 
        bty = "n", cex = 0.75)
