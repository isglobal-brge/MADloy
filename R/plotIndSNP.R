...
plotIndSNP <- function(x, sample, rsCol=1, ChrCol=2, PosCol=3, LRRCol=4, BAFCol=5, offset=0, scaleY=FALSE, ...) {
  if (inherits(x, "MADloy") | inherits(x, "MADloyBdev")) {
    ...
    graphics::plot(dat[, PosCol], dat[, LRRCol]+offset, ylim = c(-2, 2), las = 1, pch =".", cex = 2, col = 1, ylab = "",xlab = "", main = "", xaxt="n", yaxt="n", xlim=c(1, as.numeric(regions[regions$chromosome == "Y" & regions$type == "PAR2"]$end)), ...)
    if (xy){
      graphics::points(datxy[selPAR1, PosCol], datxy[selPAR1, LRRCol], pch =".", cex = 2, col=1)
      graphics::points(datxy[selPAR2, PosCol]-as.numeric(regions[regions$chromosome == "X" & regions$type == "PAR2"]$start)+as.numeric(regions[regions$chromosome == "Y" & regions$type == "PAR2"]$start), datxy[selPAR2, LRRCol], pch =".", cex = 2, col=1)
    }
    ...
    graphics::plot(dat[, PosCol], dat[, BAFCol], col = 2, pch = ".", cex = 2, ylab = "", xlab = "", main = "", axes = F, xlim=c(1, as.numeric(regions[3, 4])), ...)
    if (xy){
      graphics::points(datxy[selPAR1, PosCol], datxy[selPAR1, BAFCol], pch =".", cex = 2, col=2)
      graphics::points(datxy[selPAR2, PosCol]-as.numeric(regions[regions$chromosome == "X" & regions$type == "PAR2"]$start)+as.numeric(regions[regions$chromosome == "Y" & regions$type == "PAR2"]$start), datxy[selPAR2, BAFCol], pch =".", cex = 2, col=2)
    }
    ...
    graphics::rect(regions[regions$type=="msY"]$start, -0.01, regions[regions$type=="msY"]$end, -0.03, col="green", border=NA)
    graphics::rect(regions[regions$type=="msY"]$start, 1.01, regions[regions$type=="msY"]$end, 1.03, col="green", border=NA)
    graphics::abline(v=c(regions[regions$type=="msY"]$start, regions[regions$type=="msY"]$end), lty=2, col="green")
    graphics::rect(regions[regions$chromosome == "Y" & regions$type == "PAR1"]$start, -0.01, regions[regions$chromosome == "Y" & regions$type == "PAR1"]$end, -0.03, col="Blue", border=NA)
    graphics::rect(regions[regions$chromosome == "Y" & regions$type == "PAR1"]$start, 1.01, regions[regions$chromosome == "Y" & regions$type == "PAR1"]$end, 1.03, col="Blue", border=NA)
    graphics::abline(v=c(regions[regions$chromosome == "Y" & regions$type == "PAR1"]$start, regions[regions$chromosome == "Y" & regions$type == "PAR1"]$end), lty=2, col="blue")
    graphics::rect(regions[regions$chromosome == "Y" & regions$type == "PAR2"]$start, -0.01, regions[regions$chromosome == "Y" & regions$type == "PAR2"]$end, -0.03, col="Blue", border=NA)
    graphics::rect(regions[regions$chromosome == "Y" & regions$type == "PAR2"]$start, 1.01, regions[regions$chromosome == "Y" & regions$type == "PAR2"]$end, 1.03, col="Blue", border=NA)
    graphics::abline(v=c(regions[regions$chromosome == "Y" & regions$type == "PAR2"]$start, regions[regions$chromosome == "Y" & regions$type == "PAR2"]$end), lty=2, col="blue")
    graphics::rect(regions[regions$chromosome == "Y" & regions$type == "XTR"]$start, -0.01, regions[regions$chromosome == "Y" & regions$type == "XTR"]$end, -0.03, col="Orange", border=NA)
    graphics::rect(regions[regions$chromosome == "Y" & regions$type == "XTR"]$start, 1.01, regions[regions$chromosome == "Y" & regions$type == "XTR"]$end, 1.03, col="Orange", border=NA)
    graphics::abline(v=c(regions[regions$chromosome == "Y" & regions$type == "XTR"]$start, regions[regions$chromosome == "Y" & regions$type == "XTR"]$end), lty=2, col="Orange")
