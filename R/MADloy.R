...
madloy <- function(files, target.region, ref.region = "Autosomes", rsCol = 1, ChrCol = 2, 
    PosCol = 3, LRRCol = 4, trim = 0, mc.cores, quiet = FALSE, hg = "hg18", ...) {
    
    # Check hg version and name
    if (!hg %in% c("hg18", "hg19", "hg38")) 
        stop("The human genome release in the hg field should be one of the following ones: 'hg18', 'hg19' or 'hg38'.")
    chrSizes <- fread(system.file("extdata", "references", paste0(hg, ".chrom.sizes"), 
        package = "MADloy"), header = T, skip = "#", colClasses = c("character", 
        "numeric"), showProgress = FALSE)
    regions <- fread(system.file("extdata", "references", paste0(hg, ".par.regions"), 
        package = "MADloy"), header = T, skip = 1, colClasses = c("character", "character", 
        "numeric", "numeric"), showProgress = FALSE)
    ...
    if (missing(ref.region)) {
        ...
        subsetB <- GenomicRanges::GRanges(seqnames = chrSizes$chromosome[1:22], ranges = IRanges::IRanges(0, 
            chrSizes$size[1:22]))
    } else {
        queryB <- unlist(strsplit(x = ref.region, split = "[:, -]", perl = T))
        if (is.na(queryB[2]) | is.na(queryB[3])) {
            subsetB <- GenomicRanges::GRanges(seqnames = gsub("chr", "", queryB[1]), 
                ranges = IRanges::IRanges(start = 1, end = 3e+08))
            if (queryB[1] == "Autosomes") 
                subsetB <- GenomicRanges::GRanges(seqnames = chrSizes$chromosome[1:22], 
                  ranges = IRanges::IRanges(0, chrSizes$size[1:22]))
        } else {
            subsetB <- GenomicRanges::GRanges(seqnames = gsub("chr", "", queryB[1]), 
                ranges = IRanges::IRanges(start = as.numeric(queryB[2]), end = as.numeric(queryB[3])))
        }
    }
    
    if (!quiet) 
        if (missing(ref.region)) 
            message(paste0("LRR median will be computed in all autosomal chromosomes as reference regions and target region ", 
                target.region, "\n")) else message(paste0("LRR median will be computed in reference region ", ref.region, 
            " and target region ", target.region, "\n"))
    ...
    targetLRR <- parallel::mclapply(X = allfiles, FUN = processMAD, rsCol = rsCol, 
        ChrCol = ChrCol, PosCol = PosCol, LRRCol = LRRCol, query = subsetA, mc.cores = mc.cores, 
        trim = trim)
    ...
    par <- list(files = basename(allfiles), hg = hg, path = dirname(allfiles), cols = c(rsCol, 
        ChrCol, PosCol, LRRCol), ref.region = subsetB, regions = regions, target.region = subsetA, 
        trim = trim)
