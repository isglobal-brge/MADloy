...
MADseqLOY <- function(files, exomeTargets, target.region = "chrY:7000000-30000000", 
  ...
  #### Check target and reference region
  ...
  if (is.na(queryC[2]) | is.na(queryC[3])) {
    subsetC <- GenomicRanges::GRanges(seqnames = queryC[1], ranges = IRanges::IRanges(start = 1, 
      end = 3e+08))
  } else {
    subsetC <- GenomicRanges::GRanges(seqnames = queryC[1], ranges = IRanges::IRanges(start = as.numeric(queryC[2]), 
      end = as.numeric(queryC[3])))
  }
  if (!quiet) 
    message(paste0("Computing coverages in reference regions ", ref.region.1, 
    ...
    if (length(files) == 1) {
      if (!file.exists(files)) {
        ...
        if (file.info(files)$isdir) {
          allfiles <- grep(pattern = ".bam$", x = list.files(files, recursive = T, 
          full.names = T), perl = T, value = T)
          if (length(allfiles) == 0) 
          stop("There are no files in the given folder\n")
        } else {
          if (length(grep(".bam$", files, perl = T, value = T)) == 0) 
      ...
      if (!(length(grep(".bam$", files, perl = T, value = T)) == length(files))) {
        if (!quiet) 
          message(paste0("The file ", grep(".bam$", files, perl = T, value = T, 
          invert = T), " is not a .bam file\n"))
  ...
  #### Check exome targets input
  #' The number of lines to skip depends on the annotation file downloaded. Keep aware of it, maybe by passing another value by argument.
  #' The class of targets is GRanges and it's style is UCSC
  ...
  if (missing(exomeTargets)) {
    ...
    if (!(length(grep(".bed$", exomeTargets, perl = T, value = T)) == 1)) 
  ...
  targetCov <- parallel::mclapply(X = allfiles, FUN = processBam, targets = targets, subset = subsetA, 
    mc.cores = mc.cores)
  ref.1.Cov <- parallel::mclapply(X = allfiles, FUN = processBam, targets = targets, subset = subsetB, 
    mc.cores = mc.cores)
  ref.2.Cov <- parallel::mclapply(X = allfiles, FUN = processBam, targets = targets, subset = subsetC, 
    mc.cores = mc.cores)
  ...
  coverages <- list(target = targetCov, reference.1 = ref.1.Cov, reference.2 = ref.2.Cov, 
    par = par)
