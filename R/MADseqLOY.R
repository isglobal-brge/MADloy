...
madseqloy <- function(files, exomeTargets, target.region, 
    ...
    if (!quiet) 
        message(paste0("Computing coverages in reference region ", ref.region, " and target region ", 
            target.region, "\n"))
    ...
    targetCov <- parallel::mclapply(X = allfiles, FUN = processBam, targets = targets, 
        subset = subsetA, mc.cores = mc.cores)
    refCov <- parallel::mclapply(X = allfiles, FUN = processBam, targets = targets, 
