...
processMAD <- function(file, query, rsCol, ChrCol, PosCol, LRRCol) {
  ...
  LRRmedian <- list()
  LRRmedian$median <- median(dat$Log.R.Ratio[which(dat$Chr == as.character(GenomeInfoDb::seqnames(query)) & dat$Position > 
    BiocGenerics::start(query) & dat$Position < BiocGenerics::end(query))], na.rm = T)
  return(LRRmedian)
} 
