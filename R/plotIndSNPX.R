...
plotIndSNPX <- function(x, sample, rsCol=1, ChrCol=2, PosCol=3, LRRCol=4, BAFCol=5, offset=0, ...) {
    ...
    plot(dat[, PosCol], dat[, LRRCol]+offset, ylim = c(-2, 2), las = 1, pch =".", cex = 2, col = 1, ylab = "",xlab = "", main = "", xaxt="n", yaxt="n", xlim=c(1, as.numeric(regions[regions$chromosome == "X" & regions$type == "PAR2"]$end)), ...)
    if (xy){
      points(datxy[selPAR1, PosCol], datxy[selPAR1, LRRCol], pch =".", cex = 2, col=1)
      points(datxy[selPAR2, PosCol], datxy[selPAR2, LRRCol], pch =".", cex = 2, col=1)
    }
    axis( 2, at = c(-2:2), labels = c("-2.0", "-1.0", "0.0", "1.0", "2.0"), las = 1, col = "black", col.axis = "black")
    #abline(h=x$par$offset, col="orange", lty=2, lwd=2)
    par(new = TRUE)
    plot(dat[, PosCol], dat[, BAFCol], col = 2, pch = ".", cex = 2, ylab = "", xlab = "", main = "", axes = F, xlim=c(1, as.numeric(regions[regions$chromosome == "X" & regions$type == "PAR2"]$end)), ...)
    if (xy){
      points(datxy[selPAR1, PosCol], datxy[selPAR1, BAFCol], pch =".", cex = 2, col=2)
      points(datxy[selPAR2, PosCol], datxy[selPAR2, BAFCol], pch =".", cex = 2, col=2)
    }
    abline(h=0.5, col=8)
    ...
    rect(regions[regions$chromosome == "X" & regions$type == "PAR1"]$start, -0.01, regions[regions$chromosome == "X" & regions$type == "PAR1"]$end, -0.03, col="Blue", border=NA)
    rect(regions[regions$chromosome == "X" & regions$type == "PAR1"]$start, 1.01, regions[regions$chromosome == "X" & regions$type == "PAR1"]$end, 1.03, col="Blue", border=NA)
    abline(v=c(regions[regions$chromosome == "X" & regions$type == "PAR1"]$start, regions[regions$chromosome == "X" & regions$type == "PAR1"]$end), lty=2, col="blue")
